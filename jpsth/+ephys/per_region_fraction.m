...
fh=figure('Color','w');
cmap=colormap('jet');
collection=recurSubregions(meta,1,ismember(meta.reg_tree(1,:),{'BS','CH'}),0,1,cmap,[]);
blame=vcs.blame();
...
function collection=recurSubregions(meta,curr_branch,prev_sel,prev_accu,prev_ratio,cmap,collection)
xbound=0:0.2:1;
if curr_branch==1
    curr_ureg={'BS','CH'};
else
...
curr_ratio=cellfun(@(x) nnz(meta.mem_type(strcmp(meta.reg_tree(curr_branch,:),x))>0)./...
...
%     end
    if curr_branch<5
        collection=recurSubregions(meta,curr_branch+1,curr_sel,curr_accu,sub_ratio,cmap,collection);
    end
