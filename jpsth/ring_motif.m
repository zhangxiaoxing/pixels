...
set(gca,'XTick',[-1,1 5],'XTickLabel',{'ITI','1','5'},'YScale','log','YColor','k','FontSize',10,'Ytick',[1,100,10000]);
...
if ~evalin('base','bz')
    ...
    if inact
        ...
        fn=errorbar([-1,1:6],mean(nonp_rings(:,[1,4:9])./msize),std(nonp_rings(:,[1,4:9])./msize)./sqrt(size(nonp_rings,1)),':ro','MarkerSize',3)
...
%         fn=plot([-1,1:6],mean(nonp_rings(:,[1,4:9])),':ro','MarkerSize',3);
        ...
        fn=errorbar(1:6,nanmean(nonp_rings(:,4:9)./msize),nanstd(nonp_rings(:,4:9)./msize)./sqrt(sum(~isnan(nonp_rings(:,4:9)))),':ro','MarkerSize',3)
...
%         plot(1:6,mean(nonp_rings(:,4:9)),':ro','MarkerSize',3);
    ...
    ylabel('Norm. Ring frequency')
%     legend([fp,fn],{'Prefered','Non-pref.'})
    text(0,max(ylim()),sprintf('n=%d',size(pref_rings,1)),'Color','r');
