...
singleTracks=dir('K:\neupix\wyt\DataSum\singleProbe');
singleTrackList={singleTracks(3:end).name};
% error_list=cell(0);
sums=cell(0,8);
fs=dir('selec_XCORR*_delay_6_1_2*.mat');
for i=1:length(fs)
    tracks=2;
    disp(i);
    ...
    wffile=fullfile(['K:\neupix\DataSum\',dpath],'wf_stats.hdf5');% posix
    if isfile(wffile)
        ...
        for lblidx=1:size(xc_s1.label,1)
            ...
            if imecNo==0
                ...
                if ~isempty(wfidx)
                    ...
                    raw_wf_file=fullfile(['K:\neupix\WF\neuropixel\',dpath],'waveform.mat'); %posix
                ...
                if ~isempty(wfidx)
                    ...
                    raw_wf_file=fullfile(['K:\neupix\WF\neuropixel\',replace(dpath,'imec0','imec1')],'waveform.mat'); %posix
