% function replaced
% pending removal as of 2023.03.19

% TODO: two additional column per snapshot, filter and activation sequence
% labeling

% >> per-structure, per fcsp sequence tag, sorted by time

% TODO: isolate and encapsualte routines to obtain in_maximum_tags and
% in_trial_tags

% TODO: program-driven(gephiTK/java) snap-shot generation
...
for maxid=reshape(maxids(1:10),1,[])
    ...
    if false%bburst
        ...
        % TODO: move from fixed-wiring to time-varied snapshots
        ...
        edgecell=[{'Source','Target','Pattern'};num2cell(allconns)];
        writecell(edgecell,fullfile('bzdata',sprintf('SingleSpkConn4gephi_s%dm_%d.csv',sessid,maxid)));
