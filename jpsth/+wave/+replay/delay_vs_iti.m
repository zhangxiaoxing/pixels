...
for sess=reshape(unique([ring_replay_tbl.session;chain_replay.session]),1,[])
    ...
    for onewave=["s1d3","s1d6","s2d3","s2d6"]
        ...
        covered_tbl=[covered_tbl;cell2table({sess,onewave,sparse(covered.delay),sparse(covered.iti),sparse(covered.out_task)},'VariableNames',{'session','wave','delay','iti','out_task'})];
