...
fh=figure('Color','w');
cmap=colormap('jet');
collection=recurSubregions(1,ismember(com_meta(:,4),{'CH','BS'}),0,1,cmap,com_meta,[]);
blame=vcs.blame();
...
function collection=recurSubregions(curr_branch,prev_sel,prev_accu,prev_ratio,cmap,com_meta,collection)
xbound=0:0.2:1;
if curr_branch==1
    curr_ureg={'BS','CH'};
else
...
for curr_idx=1:numel(curr_ureg)
    ...
    patch(xbound([curr_branch,curr_branch+1,curr_branch+1,curr_branch]),[curr_accu,curr_accu,curr_accu+sub_ratio,curr_accu+sub_ratio],[-1,-1,-1,-1],cmap(com2cmapidx(curr_com_all(curr_idx)),:),'EdgeColor','none');
%     if curr_branch>3
...
%     end
    if curr_branch<5
        collection=recurSubregions(curr_branch+1,curr_sel,curr_accu,sub_ratio,cmap,com_meta,collection);
    end
    curr_accu=curr_accu+sub_ratio;
end
end

function out=com2cmapidx(com)
out=floor((com-4)/16*256)+1;
if out<1,out=1;end
