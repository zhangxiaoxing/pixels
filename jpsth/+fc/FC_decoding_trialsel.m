...
for bin=1:14
    for d=[3 6]
        ...
        fcdata.(expd(d)).nonn(bin)=nnz(all(stats.(expd(d))(:,:,2)>=pp,2)...
            & all(stats.(expd(d))(:,:,3)>=pp,2));
        fcdata.(expd(d)).pren(bin)=nnz(any(stats.(expd(d))(:,:,2)<pp,2)...
            & all(stats.(expd(d))(:,:,3)>=pp,2));
        fcdata.(expd(d)).postn(bin)=nnz(all(stats.(expd(d))(:,:,2)>=pp,2)...
            & any(stats.(expd(d))(:,:,3)<pp,2));
        fcdata.(expd(d)).bothn(bin)=nnz(any(stats.(expd(d))(:,:,2)<pp,2)...
            & any(stats.(expd(d))(:,:,3)<pp,2));
    end
end
...
for d=[3 6]
    ...
    exportgraphics(fh,'fc_coding_trialsel.pdf')
