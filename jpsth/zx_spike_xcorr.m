function [stat] = zx_spike_xcorr(cfg, spike)
rpts=200;
...
if doShiftPredictor
    s     = zeros(nChans,nChans,2*nLags,rpts);
else
...
for iTrial = 1:nTrials
    ...
    for iCmb = 1:nCmbs
        ...
        switch cfg.method
            ...
            case 'shiftpredictor'
                %% alternative shift process
                % compute the xcorr if both are non-empty
                if ~isempty(ts1) && ~isempty(ts2)
                    for rpt=1:rpts
                        ts1=ts1+rand(size(ts1))*0.01-0.005;
                        if indx(1)<=indx(2)
                            [x]   = spike_crossx_matlab(ts1(:),ts2(:),cfg.binsize,nLags*2+1);  % removed the mex file for now, until issue on windows is resolved
                        else
                            [x]   = spike_crossx_matlab(ts2(:),ts1(:),cfg.binsize,nLags*2+1); % removed the mex file for now, until issue on windows is resolved
                        ...
                        % sum the xcorr
                        s(indx(1),indx(2),:,rpt) = s(indx(1),indx(2),:,rpt) + shiftdim(x(:),-2);
                        s(indx(2),indx(1),:,rpt) = s(indx(2),indx(1),:,rpt) + shiftdim(flipud(x(:)),-2);
...
% return the results
if doShiftPredictor
    [lb99,ub99,lb95,ub95,med]=globalband(s,5,1);
    stat.(cfg.method)      = med;
    stat.globalband=cat(3,lb99,ub99,lb95,ub95);
    stat.banddimord={'lb99','ub99','lb95','ub95'};
else
    stat.(cfg.method)      = s;
end
lags = (-nLags:nLags)*cfg.binsize;
...
function [lb99,ub99,lb95,ub95,med]=globalband(in,k99,k95)

uall=squeeze(max(in,[],3));
lall=squeeze(min(in,[],3));
ub95=min(maxk(uall,k95,3),[],3);
lb95=max(mink(lall,k95,3),[],3);
ub99=min(maxk(uall,k99,3),[],3);
lb99=max(mink(lall,k99,3),[],3);
med=median(in,4);
%% TODO stats
end
