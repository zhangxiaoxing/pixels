...
if opt.overwrite || ~isfile(sprintf('net_at_%d_%s.mat',depth,opt.type))
    ...
    net_count=nan(size(netedge,1),2);
    if strcmp(opt.type,'memory')
        sigmsel=any(sig.wrsp(:,5:10,1)<0.05,2) & any(sig.wrsp(:,5:10,2)<0.05,2);
        pairmsel=any(pair.wrsp(:,5:10,1)<0.05,2) & any(pair.wrsp(:,5:10,2)<0.05,2);
    elseif strcmp(opt.type,'nonmem')
        sigmsel=~any(sig.wrsp(:,5:10,1)<0.05,2) & ~any(sig.wrsp(:,5:10,2)<0.05,2);
        pairmsel=~any(pair.wrsp(:,5:10,1)<0.05,2) & ~any(pair.wrsp(:,5:10,2)<0.05,2);
    else
        sigmsel=true(size(sig.wrsp,1),1);
        pairmsel=true(size(pair.wrsp,1),1);
    end
    ...
    net_count(:,3)=net_count(:,2)./net_count(:,1);
