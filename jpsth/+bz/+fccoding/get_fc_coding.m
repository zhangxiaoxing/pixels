...
if isempty(metas_) || isempty(stats_) || ~isequaln(opt_,opt) || opt.force_update
    ...
    fl=dir(fullfile('binary','fccoding','fc_coding_*.mat')); % data source jpsth\+bz\+fccoding\fc_coding_one_sess.m
    ...
    if strcmp(opt.type,'olf')
        delays=[3,6];
    else
        delays=[-1]; % placeholder
    end
    for fi=1:numel(fl)
        for delay=delays
            ...
            switch opt.type
                case 'olf'
                    if opt.incong
                        inconsel=pct.su_pairs.get_incongru(waveids,'odor_only',opt.odor_only);
                        if delay==3
                            fcIdces=find(inconsel & all(ismember(waveids,[1 3 5 6]),2));
                        else
                            fcIdces=find(inconsel & all(ismember(waveids,[2 4 5 6]),2));
                        end
                    else
                        congrusel=pct.su_pairs.get_congru(waveids,'odor_only',opt.odor_only);
                        if delay==3
                            fcIdces=find(congrusel & all(ismember(waveids,[1 3 5 6]),2));
                        else
                            fcIdces=find(congrusel & all(ismember(waveids,[2 4 5 6]),2));
                        end
                    end
                    % TODO: coud distinguish 3s-prefer and 6s-prefer SC and
                    % trials at this step, but not enacting for the fair
                    % comparison with incongruent FCs.

                    lbl1sel=fstr.onesess.trials(:,5)==4 & ismember(fstr.onesess.trials(:,8),delay) & all(fstr.onesess.trials(:,9:10),2);
                    lbl2sel=fstr.onesess.trials(:,5)==8 & ismember(fstr.onesess.trials(:,8),delay) & all(fstr.onesess.trials(:,9:10),2);
                    e1sel=fstr.onesess.trials(:,5)==4 & ismember(fstr.onesess.trials(:,8),delay) & fstr.onesess.trials(:,10)==0;
                    e2sel=fstr.onesess.trials(:,5)==8 & ismember(fstr.onesess.trials(:,8),delay) & fstr.onesess.trials(:,10)==0;
                case 'dur'
                    if opt.incong
