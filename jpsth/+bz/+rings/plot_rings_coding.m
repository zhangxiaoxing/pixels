%bool_congru+,bool_cross+,sess,rsize,loops_id,cids (+up to 2 fill-in),
%cid,delay correct rate,iti correct rate, delay error rate
function plot_rings_coding(opt)
arguments
    opt.plot_histogram (1,1) logical = false
    opt.plot_bars (1,1) logical = false
end
        ...
        for rtype=regtypes
            ...
            for li=1:numel(loops)
                ...
                for ci=1:numel(loops(li).label)
                    ...
                    rings_coding_sums=[rings_coding_sums;...
...
per_trl=[rings_coding_sums(:,12)./rings_coding_sums(:,13),rings_coding_sums(:,14)./rings_coding_sums(:,15),...
    rings_coding_sums(:,16)./rings_coding_sums(:,17),rings_coding_sums(:,18)./rings_coding_sums(:,19),...
    rings_coding_sums(:,20)./rings_coding_sums(:,21),rings_coding_sums(:,22)./rings_coding_sums(:,23)];
...
%max per trial event >0.5, min (correct | error) trial >=5
freq_sel=max(per_trl(:,1:2),[],2)>=0.5 & min(rings_coding_sums(:,[13,15,21,23]),[],2)>=5;
%% selectivity index distribution histogram
congru_sel=rings_coding_sums(:,1)==1;
if opt.plot_histogram
