...
close all
load('reg_keep.mat');
major=cellfun(@(x) ~isempty(regexp(x,'[A-Z]','match','once')), reg_set);
% reg_set=reg_set(~strcmp(reg_set,'Unlabeled'));
% reg_set=reg_set(~strcmp(reg_set,'root'));
% reg_set=reg_set(major);
plot_per_bin=false;
plot_entire=true;
...
if plot_entire
    plotOne(io_entire_delay,reg_set,'sum of all bins','0714_io_selec_bin_sum.png',[4,5,8,9],regclass);
    plotOne(io_entire_delay,reg_set,'sum of all bins','0714_io_conn_bin_sum.png',[2,3,6,7],regclass);
end
...
function plotOne(in_out_sel,reg_set,str_title,fname,v_idx,regclass)
    ...
    major=cellfun(@(x) ~isempty(regexp(x,'[A-Z]','match','once')), reg_set);
    countsel=in_out_sel(:,1)>100 & major;
    ...
    hold on
%     scatter(iosel(~(sink_sel|source_sel),inFrac),iosel(~(sink_sel|source_sel),outFrac),'o','MarkerEdgeColor','none','MarkerFaceColor',[0.5,0.5,0.5],'MarkerFaceAlpha',0.5);
%     scatter(iosel(source_sel,inFrac),iosel(source_sel,outFrac),'o','MarkerEdgeColor','none','MarkerFaceColor','r','MarkerFaceAlpha',0.5);
%     scatter(iosel(sink_sel,inFrac),iosel(sink_sel,outFrac),'o','MarkerEdgeColor','none','MarkerFaceColor','b','MarkerFaceAlpha',0.5);
