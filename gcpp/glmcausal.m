function [beta_new] = glmcausal(X,n,trigger,ht,k,w)

%================================================================
%    GLM fitting based on submatrices after excluding the effect
%    of the spiking history of trigger neuron
%================================================================
%
%  This code is made for the case when input matrix Y is large
%  X is partioned into many small submatrices of (k x 1)-dimension
%  This code is based on bnlrCG.m (Demba) and 
%
%   References:
%      Dobson, A.J. (1990), "An Introduction to Generalized Linear
%         Models," CRC Press.
%      McCullagh, P., and J.A. Nelder (1990), "Generalized Linear
%         Models," CRC Press.
%
% Input arguments:
%           X: measurement data (K x Q)
%           n: index number of input (target) neuron to analyze
%     trigger: index number of a neuron whose effect will be excluded
%          ht: model order (using AIC or BIC)
%           k: one of divisors for K
%           w: duration of non-overlapping spike counting window
%
% Output arguments:
%     beta_new: estimated GLM parameters (p x 1)
%
%
%================================================================
% Sanggyun Kim
% Neuroscience Statistics Research Lab (BCS MIT)
% April 13. 2009
%================================================================

% Counting window
WIN = zeros(ht/w,ht);
for iwin = 1:ht/w
    WIN(iwin,(iwin-1)*w+1:iwin*w) = 1;
end

% CG parameters
cgeps = 1e-3;
cgmax = 30;

% LR parameters
Irmax = 100;
Ireps = 0.05;

Q = size(X,2);
% Removing trigger neuron
if trigger == 1
    Xc = X(:,trigger+1:end);
elseif trigger == Q
    Xc = X(:,1:trigger-1);
else
    Xc = [X(:,1:trigger-1) X(:,trigger+1:end)];
end

% Making covariate matrix Xsub{}
Xcnew = Xc(ht+1:end,:);
[K Q] = size(Xcnew);
p = Q*ht/w +1;
for kk = 1:K/k
    for ii = 1:k-ht
        temp = [1];
        for jj = 1:Q
            temp = [temp (WIN*Xc(ht+(kk-1)*k+ii-1:-1:(kk-1)*k+ii,jj))']; 
        end
        Xsub{kk,1}(ii,1:p) = temp;
    end
end

% Making output matrix Ysub{}
Ynew = X(ht+1:end,:);
for kk = 1:K/k
    Ysub{kk} = Ynew(k*(kk-1)+1:k*kk-ht,n);
end

% Logistic regression
i = 0;
% Initialization
P = length(Xsub{1,1});

beta_old = zeros(p,1);
% W = {Wsub{kk}} & z = {zsub{kk}}
for kk = 1:K/k
%     eta{kk} = zeros(k,1);
    eta{kk} = Xsub{kk}*beta_old(1:p);
    musub{kk} = exp(eta{kk})./(1+exp(eta{kk}));
    Wsub{kk} = diag(musub{kk}).*diag(1-musub{kk});
    zsub{kk} = eta{kk} + (Ysub{kk}-musub{kk}).*(1./diag(Wsub{kk}));
end

% Scaled deviance
devold = 0;
for kk = 1:K/k
    devold = devold - 2*(Ysub{kk}'*log(musub{kk})+(1-Ysub{kk})'*log(1-musub{kk}));
end
devnew = 0;
devdiff = abs(devnew - devold);

% Do CG -> beta_new, i.e. solve for beta_new: X'WX*beta_new =
% X'Wz(beta_old) using CG
while (i < Irmax && devdiff > Ireps)

    A(1:p,1:p) = zeros(p,p);
    for kk = 1:K/k
        A(1:p,1:p) = A(1:p,1:p) + Xsub{kk}'*Wsub{kk}*Xsub{kk};
    end

    %A = A + A' - diag(diag(A));

    
    b(1:p,1) = zeros(p,1);
    for kk = 1:K/k
        b(1:p,1) = b(1:p,1) + Xsub{kk}'*Wsub{kk}*zsub{kk};
    end
    

    % Conjugate gradient method for symmetric postive definite matrix A
    beta_new = cgs(A,b,cgeps,cgmax,[],[],beta_old);
    beta_old = beta_new;

    for kk = 1:K/k
%         eta{kk} = zeros(k,1);
        eta{kk} = Xsub{kk}*beta_old(1:p);
        musub{kk} = exp(eta{kk})./(1+exp(eta{kk}));
        Wsub{kk} = diag(musub{kk}).*diag(1-musub{kk});
        zsub{kk} = eta{kk} + (Ysub{kk}-musub{kk}).*(1./diag(Wsub{kk}));
    end

    % Scaled deviance
    devnew = 0;
    for kk = 1:K/k
        devnew = devnew - 2*(Ysub{kk}'*log(musub{kk})+(1-Ysub{kk})'*log(1-musub{kk}));
    end
    devdiff = abs(devnew - devold);
    devold = devnew;
    
    i = i+1;
    
end

% % Compute additional statistics
% stats.dfe = 0;
% stats.s = 0;
% stats.sfit = 0;
% stats.covb = inv(A);
% stats.se = sqrt(diag(stats.covb));
% stats.coeffcorr = stats.covb./sqrt((repmat(diag(stats.covb),1,p).*repmat(diag(stats.covb)',p,1)));
% stats.t = 0;
% stats.p = 0;
% stats.resid = 0;
% stats.residp = 0;
% stats.residd = 0;
% stats.resida = 0;