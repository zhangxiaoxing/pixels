...
def gen_data():
    ...
    ### Actual data accessed after this line
            ...
            for t in s1c_t:
                s1corr.append(hist[t+1:t+7])
            for t in s2c_t:
                s2corr.append(hist[t+1:t+7])
            for t in s1e_t:
                s1err.append(hist[t+1:t+7])
            for t in s2e_t:
                s2err.append(hist[t+1:t+7])
...
def gen_mat(data,STP_n=100,corr_trial=20,err_trial=2):
   nn=np.random.choice(data,STP_n,replace=False)
   out=[{'s1corr':[n['s1corr'][x] for x in np.random.choice(len(n['s1corr']),corr_trial,replace=False)],
...
def decode(dec_data):
    ...
    err_dec=[]
    for t in range(6):
        X1=np.moveaxis(np.array([n['s1corr'] for n in dec_data]),[0,1],[1,0])[:,:,t]
        X2=np.moveaxis(np.array([n['s2corr'] for n in dec_data]),[0,1],[1,0])[:,:,t]
        X1err=np.moveaxis(np.array([n['s1err'] for n in dec_data]),[0,1],[1,0])[:,:,t]
        X2err=np.moveaxis(np.array([n['s2err'] for n in dec_data]),[0,1],[1,0])[:,:,t]
        ...
        dec.append(cross_val_score(clf, X, Y, cv=10, n_jobs=-1) * 100)
        shuf_dec.append(cross_val_score(clf, X, Y_shuf, cv=10, n_jobs=-1) * 100)
        clf.fit(X, Y)
        err_dec.append(clf.score(Xerr, Yerr)*100)
        
    return (dec,shuf_dec,err_dec)
...
def gen_dec_arr(STP_n=100):
    ...
    for i in range(rpt):
        ...
        (dec,shuf_dec,err_dec)=decode(congru_data)
        ...
        (inc_dec,inc_shuf_dec,inc_err_dec)=decode(incongru_data)
    ...
    cong_err_arr=np.vstack(tuple(congru_err)).transpose()
    incong_arr=np.hstack(tuple(incongru_dec))
    incong_shuf_arr=np.hstack(tuple(incongru_shuf))
    incong_err_arr=np.vstack(tuple(incongru_err)).transpose()
    pickle.dump({'congrur':cong_arr,'congru_shuf':cong_shuf_arr,'congru_err':cong_err_arr,
                 'incongru':incong_arr,'incongru_shuf':incong_shuf_arr,'incongru_err':incong_err_arr},
