...
def gen_selectivity_stats(delay, debug = False, denovo = True, complete=False):
    '''
    Generate per SU selectivity and brain region tree file
    '''
    if denovo:
        (dict_stats,error_files)=prepare_data(delay = delay, debug = debug, complete=complete)
    ...
    exporth5py(dict_stats,complete=complete) # transient_{delay}.hdf5
    return error_files
