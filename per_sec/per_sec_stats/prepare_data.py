...
def prepare_data(delay=6,debug=False):
    ...
    for path in sorted(futil.traverse(dpath)):
        ...
        with h5py.File(os.path.join(path, "FR_All_1000.hdf5"), "r") as ffr:
            ...
            WF_good = np.array(ffr["WF_good"], dtype="int8")
        ...
        if not os.path.isfile(os.path.join(path, "su_id2reg.csv")):
            error_files.append(['Missing localization',path])
            continue
        ...
        dict_stats=get_stats(trial_FR, trials, delay=delay, debug=debug) #statistics
        ...
        if not np.all([reg_suids[x]==SU_ids[0][x] for x in range(len(reg_suids))]):
            breakpoint()
